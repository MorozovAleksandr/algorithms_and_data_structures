...
- j - количество уникальных слов в запросе
- d - среднее количество документов, содержащих слово
- s - количество найденных релевантных документов

Построение инвертированного индекса: O(n * m)
Обработка одного запроса в среднем: O(j * d + s)
Обработка одного запроса в худшем(слово есть в каждом документе): O(j * n + s)
Обработка всех запросов: O(k * (j * n + s))

Итоговая временная сложность: O(n * m + k * (j * n + s))
