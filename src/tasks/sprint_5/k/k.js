...
function getBtsRange(root, left, right, result = []) {
    if (root.left && root.value >= left) {
        getBtsRange(root.left, left, right, result);
    }
    ...
    if (root.right && root.value <= right) {
        getBtsRange(root.right, left, right, result);
    }
...
function printRange(root, left, right) {
    const result = getBtsRange(root, left, right);
